...
class CategoricalContinuousColorSchemeTest(unittest.TestCase):
    ...
    def test_get_extended_color_scheme(self):
        cccs = CategoricalContinuousColorScheme(
            "test_ccs", "test_ccm", [0, 10, 15], ["red", "green", "blue"], ["First", "Second", "Third"], "Reds"
        )

        cont_values = [20, 22, 23, 26]
        ecs = cccs.get_extended_color_scheme(cont_values, range_extent=5, label_shift=15, labels_space=2)

        self.assertNotEqual("test_ccm_base", ecs.color_scheme_name)
        self.assertTrue(ecs.color_scheme_name.startswith("test_ccs_"))
        self.assertNotEqual("test_ccm_base", ecs.color_map_name)
        self.assertTrue(ecs.color_map_name.startswith("test_ccm_"))
        nt.assert_array_equal(np.array([0, 10, 15, 20, 22, 23, 26]), ecs.values)
        self.assertEqual(["First", "Second", "Third", "", "7", "", "11"], ecs.labels)
        self.assertEqual(["red", "green", "blue"], ecs.colors[:3])
        self.assertEqual(7, len(ecs.colors))
        nt.assert_array_equal(np.array([2.25, 8.75, 15.0, 19.25, 21.75, 23.5, 25.5]), ecs.tick_values)
...
class ColorSchemeRegistryTest(unittest.TestCase):
    ...
    def test_register_and_get_categorical_continuous_scheme(self):
        ...
        ec_scheme = cc_scheme.get_extended_color_scheme([20, 30, 35, 40], range_extent=6, label_shift=-5, labels_space=1)

        self.assertNotEqual("test_cccs_2", ec_scheme.color_scheme_name)
        self.assertTrue(ec_scheme.color_scheme_name.startswith("test_cccs_2"))
        self.assertNotEqual("test_ccm_2_base", ec_scheme.color_map_name)
        self.assertTrue(ec_scheme.color_map_name.startswith("test_ccm_2"))
        self.assertEqual(["First", "Second", "Third", "25", "35", "40", "45"], ec_scheme.labels)
        self.assertEqual(7, len(ec_scheme.colors))
        self.assertEqual(["red", "green", "blue"], ec_scheme.colors[:3])
        nt.assert_array_equal(np.array([0, 10, 15, 20, 30, 35, 40]), ec_scheme.values)
        nt.assert_array_equal(np.array([2.25, 8.75, 15., 21.25, 28.75, 35., 39.]), ec_scheme.tick_values)
