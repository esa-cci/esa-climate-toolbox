...
class TestDsArithmetics(TestCase):
    ...
    def test_nominal(self):
        ...
        actual = arithmetics.arithmetics(dataset, '+2, -2, *3, /3, *4')
        assert_dataset_equal(expected * 4, actual)

        actual = arithmetics.arithmetics(
            dataset, 'exp, log, *10, log10, *2, log2, +1.5'
        )
        assert_dataset_equal(expected * 2.5, actual)

        actual = arithmetics.arithmetics(dataset, 'exp, -1, log1p, +3')
        assert_dataset_equal(expected * 4, actual)

        with self.assertRaises(ValueError) as err:
            arithmetics.arithmetics(dataset, 'not')
